...
# Test de friedman para ver si existen diferencias significativas entre los niveles de ciclos
import pingouin as pg
pg.friedman(data=df_summary, dv="accuracy_mean_mean", within="n_cycle", subject="combo_alpha_split")


from nonparametrictest import friedman_test, holm_test
...
# Agregamos por la media
df_summary2 = df_summary[['n_cycle','combo_alpha_split','accuracy_mean_mean']].groupby(['n_cycle','combo_alpha_split'], as_index=False).mean()
df_summary2.groupby('n_cycle').mean()
df_summary22 = df_summary2.pivot(index='combo_alpha_split', columns='n_cycle', values='accuracy_mean_mean')
aa = df_summary22.loc['alpha10-split1',:]

data_f = np.asarray(aa)
data_f_median = np.asarray(data_norem_wide2_median)


alpha = 0.05 # Set this to the desired alpha/signifance level

stat, p = st.friedmanchisquare(*data_f)
print(p)
...
statistic, p_value, ranking, rank_cmp  = friedman_test(*np.transpose(data_f))
ranks = {key: rank_cmp[i] for i, key in enumerate(list(data_f.columns))}

comparisons, z_values, p_values, adj_p_values = holm_test(ranks, control="0_test_mean")
