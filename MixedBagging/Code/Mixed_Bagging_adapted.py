            ...
            for defnumbags in defnumbags_v:
                ...
                for run in range(numruns):
                    ...
                    for tr_val_idx, test_idx in skf.split(X, y):
                        ...
                        cv_acc_gr = np.zeros(
                            (numcvruns * numcvfolds, len(hardness_intervals)))  # one for each interval within this cv fold
                        ...
                        for cvrun in range(numcvruns):
                            ...
                            for tr_idx, val_idx in skf2.split(X_tr_val, Y_tr_val):
                                ...
                                for h_i in hardness_intervals:
                                    grad_bags.append(create_gradually_mixed_bags(trainset, ih_tr, low_bag_hardness=h_i[0],
                                ...
                                for i in range(numhqs * numratios):
                                    ...
                                    cv_acc_mx[cvrun * numcvfolds + cvfold][i] = mt.accuracy_score(Y_validate,
                                                                                                  mix_final_pred[i])
                                    cv_f1_mx[cvrun * numcvfolds + cvfold][i] = mt.f1_score(Y_validate, mix_final_pred[i],
                                                                                           average='weighted')
                                ...
                                for i in range(len(hardness_intervals)):
                                    ...
                                    cv_acc_gr[cvrun * numcvfolds + cvfold][i] = mt.accuracy_score(Y_validate,
                                                                                                  grad_final_pred[i])
                                    cv_f1_gr[cvrun * numcvfolds + cvfold][i] = mt.f1_score(Y_validate, grad_final_pred[i],
                                                                                           average='weighted')
                        ...
                        mixed_bags_f1 = create_mixed_bags(trainvalset, ih, mix_ratio=mix_ratios[idx_max_f1_mx % numratios],
                                                          hard_quotient=hqs[
                        ...
                        grad_bags_acc = create_gradually_mixed_bags(trainvalset, ih,
                                                                    low_bag_hardness=hardness_intervals[idx_max_acc_gr][0],
                                                                    high_bag_hardness=hardness_intervals[idx_max_acc_gr][1])
                        grad_bags_f1 = create_gradually_mixed_bags(trainvalset, ih,
                                                                   low_bag_hardness=hardness_intervals[idx_max_f1_gr][0],
                                                                   high_bag_hardness=hardness_intervals[idx_max_f1_gr][1])
                        ...
                        res_dict_wagging_acc = {'dataset': dataset_name, 'CV_fold': fold, 'n_trees': defnumbags,
                                        ...
                                        'perf_measure': 'acc', 'perf_value': accuracy_wag[run * numfolds + fold],
                        ...
                        res_dict_boosting_acc = {'dataset': dataset_name, 'CV_fold': fold, 'n_trees': defnumbags,
                                        ...
                                        'perf_measure': 'acc', 'perf_value': accuracy_boost[run * numfolds + fold],
                        ...
                        res_dict_mix_acc = {'dataset': dataset_name, 'CV_fold': fold, 'n_trees': defnumbags,
                                        ...
                                        'parameters': [mix_ratios[idx_max_acc_mx % numratios], hqs[idx_max_acc_mx // numratios]],
                        ...
                        res_dict_mix_f1 = {'dataset': dataset_name, 'CV_fold': fold, 'n_trees': defnumbags,
                                        'model': 'Grouped_Mixed_Bagging',
                                           'parameters': [mix_ratios[idx_max_f1_mx % numratios], hqs[idx_max_f1_mx // numratios]],
                        ...
                        res_dict_mix_auc = {'dataset': dataset_name, 'CV_fold': fold, 'n_trees': defnumbags,
                                        'model': 'Grouped_Mixed_Bagging',
                                            'parameters': [mix_ratios[idx_max_auc_mx % numratios], hqs[idx_max_auc_mx // numratios]],
                ...
                df_aggre_mean = results_df.groupby(['dataset', 'n_trees','model','perf_measure'], as_index=False)['perf_value'].mean()
                df_aggre_mean.columns = ['dataset', 'n_trees','model','perf_measure','perf_value_mean']
                df_aggre_std = results_df.groupby(['dataset', 'n_trees','model','perf_measure'], as_index=False)['perf_value'].std()
                df_aggre_std.columns = ['dataset', 'n_trees','model','perf_measure','perf_value_std']
                df_aggre_conf_matrix = results_df.groupby(['dataset', 'n_trees', 'model', 'perf_measure'], as_index=False)[
                    'Conf_Matrix'].sum()
